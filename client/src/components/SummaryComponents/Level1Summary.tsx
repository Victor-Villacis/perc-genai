...
const StyledTopicContainer = styled.div.attrs<TopicContainerProps>(props => ({
    isActive: props.isActive // Explicitly handling the prop
})) <TopicContainerProps>`
...
const Level1Summary: React.FC<Level1SummaryProps> = memo(({ activeSection, data }) => {
            ...
            {data && Object.keys(data).map((mainSection, idx) => (
                <div key={`${mainSection}-${idx}`}>
                    ...
                    {Object.keys(data[mainSection]).map((subSection, idx) => (
                        <TopicContainer
                            key={`${subSection}-${idx}`}
                            isActive={activeSection === subSection}  // changed 'active' to 'isActive'
                        >
