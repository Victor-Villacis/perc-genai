...
def map2text(outlist, titles=None):
    titles = titles or ["Disease", "Experience/opinion/knowledge", "Quote 1", "Quote 2", "others"]
    outstr = ""
    ...
    for x, y in outlist.items():
        if x != "NA" and x is not None:
            ...
            if isinstance(y, dict):
                for k, v in y.items():
                    if isinstance(v, str):
                        outstr += textwrap.indent(str(k) + ": ", " " * 10) + str(v) + "\n"
                    elif v != "NA" and isinstance(v, list):
                        outstr += textwrap.indent("Name: ", " " * 10) + str(k) + "\n"
                        for i, element in enumerate(v):
                            if i < len(titles):
                                outstr += textwrap.indent(titles[i] + ": ", " " * 15) + str(element) + "\n"
...
def summarize_file(filename, timestamp, folder, P_1_L_1, P_1_L_2):
    ...
    sections = [x.replace("[//", "").replace("//]", "") for x in re.findall(r"\[//.*?//\]", text)]
...
def main():
    ...
    level1, level2 = summarize_file(file_path, dt_string, outputfolder, prompt_1_level_1, prompt_1_level_2)

    # Print and save Level 1 and Level 2 summaries
    print(json.dumps(level1, indent=4))
    print("********************")
    print(json.dumps(level2, indent=4))

    # Write summaries to output folder
