...
input_query_path <- "/home/victorv/perc-genai/server/src/services/query_input/"
...
gpt35_completion_tokens_tracker <- function(system_prompt = "",
  ...
  api_base <- "https://genaiapimna.jnj.com/openai-chat"
  deployment_id <- engine
  api_version <- "2023-03-15-preview"
  api_key <- "228b7c3ed183460abb331208fd2893b3"
...
dir_path <- "/home/victorv/perc-genai/server/src/services/input_folder/"
...
docs <- list.files(dir_path, pattern = ".docx") %>%
...
final_gpt_response <-
  gpt35_completion_tokens_tracker(
    system_prompt = default_system_prompt,
...
refined_gpt_response$output %>%
  jsonlite::toJSON() %>%
  writeLines("/home/victorv/perc-genai/server/src/services/query_output/gpt_search_out.json")
